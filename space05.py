...
pygame.display.set_caption("우주 정복하기")
...
class Laser:
    ...
    def off_screen(self, height):
        return not(self.y <= SCREEN_HIGHT and self.y >= 0)
...
class Ship:
    ...
    def move_lasers(self, vel, obj):
        ...
        for laser in self.lasers:
            laser.move(vel)
            if laser.off_screen(SCREEN_HIGHT):
...
def main():
    ...
    while run:
        ...
        if keys[pygame.K_w] and player.y - player_vel >= 0:
            player.y -= player_vel
        if keys[pygame.K_s] and player.y + player_vel <= SCREEN_HIGHT - player.get_height():
            player.y += player_vel
        ...
        for enemy in enemies:
            ...
            if enemy.y + enemy_vel > SCREEN_HIGHT:
